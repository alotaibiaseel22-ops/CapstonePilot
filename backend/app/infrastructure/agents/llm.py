...
# "gemini/" routes crewai.LLM to its native Gemini provider (crewai.llms.providers
# .gemini.completion.GeminiCompletion), which is backed by the official Google
# GenAI SDK ("google-genai" / `from google import genai`) - not litellm, not
# OpenRouter. That provider reads GOOGLE_API_KEY/GEMINI_API_KEY from the
# environment automatically, but api_key is still passed explicitly below so
# behavior doesn't depend on ambient process env vars.
_MODEL = f"gemini/{settings.GEMINI_MODEL}"
...
def documentation_analysis_llm() -> LLM:
